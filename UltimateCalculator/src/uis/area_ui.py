## @file area_ui.py
#  @brief Provides a class to display the Area window
#  @date March 18, 2022

from PyQt5.QtWidgets import QMainWindow, QApplication
from PyQt5.uic import loadUi
from uis.Calculators.geometry_calculator import getArea

## @brief AreaWindow is a class that implements the GUI components for the Area operation

class AreaWindow(QMainWindow):

    ## @brief The constructor of the Area window
    ...
    def __init__(self, path=""):
        ...
        self.path = f"{path}Ui_Base/area.ui"
        ...
        self.go_btn.clicked.connect(self.area)
        self.exit_btn.clicked.connect(self.close)
        self.clr_btn.clicked.connect(self.clearFields)
        self.comboBox.currentTextChanged.connect(self.setFields)
        
    ## @brief Displays the area of selected shape given appropriate side lengths/radius
    ...
    def area(self):
        try:
            ...
            if shape == "Square":
                ...
                val = getArea(0, side_a, None, None, None)
            elif shape == "Rectangle":
                side_a = float(self.line_2.text())
                side_b = float(self.line_3.text())
                val = getArea(1, side_a, side_b, None, None)
            elif shape == "Triangle":
                side_a = float(self.line_2.text())
                side_b = float(self.line_3.text())
                side_c = float(self.line_4.text())
                val = getArea(2, side_a, side_b, side_c, None)
            elif shape == "Circle":
                radius = float(self.line_5.text())
                val = getArea(3, None, None, None, radius)
