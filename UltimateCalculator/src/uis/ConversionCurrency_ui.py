## @file ConversionCurrency_ui.py
#  @brief Provides a class to display the currency conversion window
#  @date March 18, 2022

from PyQt5.QtWidgets import QMainWindow, QApplication
from PyQt5.uic import loadUi
from uis.Calculators.conversion_calculator import convertCurrency

## @brief ConversionBaseWindow is a class that implements the GUI components for the currency conversion operation

class ConversionCurrencyWindow(QMainWindow):

    ## @brief The constructor of the currency conversion window
    ...
    def __init__(self, path=""):
        super(ConversionCurrencyWindow, self).__init__()

        self.path = f"{path}Ui_Base/conversionCurrency.ui"
        loadUi(self.path, self)

        self.setFixedSize(421, 483)

        self.cvt_btn.clicked.connect(self.currconvert)
...
if __name__ == "__main__":
    ...
    window = ConversionCurrencyWindow()
    window.show()
    sys.exit(app.exec_())
