...
\begin{description}
\item[\refstepcounter{acnum} \actheacnum \label{acOp}:] A new operation type is added \sout{(main module UI)}.
\item[\refstepcounter{acnum} \actheacnum \label{acCon}:] A new converter is added \sout{(converter menu UI)}.
\item[\refstepcounter{acnum} \actheacnum \label{acAlg}:] A new algebraic operation is added \sout{(algebra menu UI)}.
\item[\refstepcounter{acnum} \actheacnum \label{acStk}:] A new stock operation is added \sout{(stock menu UI)}.
\item[\refstepcounter{acnum} \actheacnum \label{acGPA}:] A new GPA calculator based on a different scale is added \sout{(GPA menu UI)}.
\item[\refstepcounter{acnum} \actheacnum \label{acGeo}:] A new geometric operation is added \sout{(geometry menu UI)}.
\item[\refstepcounter{acnum} \actheacnum \label{acBin}:] A new binary operation is added \sout{(binary menu UI)}.
\item[\refstepcounter{acnum} \actheacnum \label{acHlth}:] A new health operation is added \sout{(health menu UI)}.
\item[\refstepcounter{acnum} \actheacnum \label{acAlg1}:] A change in the algorithm for algebraic operations \sout{(Algebra functionality module)}.
\item[\refstepcounter{acnum} \actheacnum \label{acAlg2}:] A change in the algorithm for GPA operations \sout{(GPA functionality)}.
\item[\refstepcounter{acnum} \actheacnum \label{acAlg3}:] A change in the algorithm for geometric operations \sout{(geometric functionality)}.
\item[\refstepcounter{acnum} \actheacnum \label{acAlg4}:] A change in the algorithm for binary operations \sout{(binary functionality)}.
\item[\refstepcounter{acnum} \actheacnum \label{acAlg5}:] A change in the algorithm for health operations \sout{(health functionality)}.
...
\item [\refstepcounter{ucnum} \uctheucnum \label{ucI}:]\sout {Changes related to the use of the internet within the application.}
\end{description}
